...
//-----------------------------------------------------------------------------
void akp_check_state::encode_frame_label(const TDataPocket &data)
{
    frame_label = take_from_14th_bit(frame_pos_frame_label, data.data);
//    qDebug() << QString("%1").arg(frame_label, 4, 16, QChar('0')).toUpper();
}
//-----------------------------------------------------------------------------
void akp_check_state::encode_vk_number(const TDataPocket &data)
{
    old_vk_number = vk_number;
    vk_number     = take_from_14th_bit(frame_pos_num, data.data);
}
//-----------------------------------------------------------------------------
...
void akp_check_state::encode_delay_value(const TDataPocket &data)
{
    ...
    rx_delay     = take_from_14th_bit(frame_pos_delay, data.data);
}
//-----------------------------------------------------------------------------
void akp_check_state::encode_Ku_value(const TDataPocket &data)
{
    old_Ku = Ku;
    Ku     = take_from_14th_bit(frame_pos_Ku, data.data);
}
//-----------------------------------------------------------------------------
void akp_check_state::encode_tool_type(const TDataPocket &data)
{
    old_tool_type = tool_type;
    tool_type     = take_from_14th_bit(frame_pos_tool_type, data.data);
}
//-----------------------------------------------------------------------------
void akp_check_state::encode_mode_number(const TDataPocket &data)
{
    old_mode_number = mode_number;
    old_mode_count  = mode_count;

    mode_number     = take_from_14th_bit(frame_pos_mode_num, data.data);
    mode_count      = take_from_14th_bit(frame_pos_mode_count, data.data);
}
//-----------------------------------------------------------------------------
void akp_check_state::encode_calibration_vk(const TDataPocket &data)
{
    old_vk_calibration_amplitude = vk_calibration_amplitude;
    old_vk_calibration_offset    = vk_calibration_offset;

    vk_calibration_amplitude     = take_from_14th_bit(frame_pos_calibr_ampl, data.data);
    vk_calibration_offset        = take_from_14th_bit(frame_pos_calibr_ofset, data.data);
}
//-----------------------------------------------------------------------------
void akp_check_state::encode_tool_no(const TDataPocket &data)
{
    old_tool_no = tool_no;
    tool_no     = take_from_14th_bit(frame_pos_tool_no, data.data);
}
//-----------------------------------------------------------------------------
void akp_check_state::encode_soft_version(const TDataPocket &data)
{
    old_soft_version_major = soft_version_major;
    old_soft_version_minor = soft_version_minor;

    soft_version_major     = take_from_14th_bit(frame_pos_ver_major, data.data);
    soft_version_minor     = take_from_14th_bit(frame_pos_ver_minor, data.data);
}
//-----------------------------------------------------------------------------
void akp_check_state::encode_timer_clk(const TDataPocket &data)
{
    TWORD32_BYTES x;

    x.word16.lo = take_from_14th_bit(frame_pos_timer_clk_lo, data.data);
    x.word16.hi = take_from_14th_bit(frame_pos_timer_clk_hi, data.data);

    old_timer_clk = timer_clk;
    timer_clk     = x.word32;
}
//-----------------------------------------------------------------------------
void akp_check_state::encode_time_start_meserment(const TDataPocket &data)
{
    TWORD32_BYTES x;

    x.word16.lo = take_from_14th_bit(frame_pos_time_start_meserment_lo, data.data);
    x.word16.hi = take_from_14th_bit(frame_pos_time_start_meserment_hi, data.data);

    old_time_start_meserment = time_start_meserment;
    time_start_meserment = x.word32;
}
//-----------------------------------------------------------------------------
void akp_check_state::encode_time_stop_meserment(const TDataPocket &data)
{
    TWORD32_BYTES x;

    x.word16.lo = take_from_14th_bit(frame_pos_time_stop_meserment_lo, data.data);
    x.word16.hi = take_from_14th_bit(frame_pos_time_stop_meserment_hi, data.data);

    old_time_stop_meserment = time_stop_meserment;
    time_stop_meserment = x.word32;
}
//-----------------------------------------------------------------------------
void akp_check_state::check_CRC(const TDataPocket &data)
{
    quint16 CRC_read;
    quint16 CRC_calc;
    //-------------------------------------------------------
    ...
    //-------------------------------------------------------
    old_CRC1_OK = CRC1_OK;
    CRC_read = take_from_15th_bit(frame_pos_CRC1, data.data);
    CRC_calc = calc_CRC(CRC1_win_left, data.data);
    ...
    //-------------------------------------------------------
    old_CRC2_OK = CRC2_OK;
    CRC_read = take_from_15th_bit(frame_pos_CRC2, data.data);
    CRC_calc = calc_CRC(CRC2_win_left, data.data);
    ...
    //-------------------------------------------------------
    old_CRC3_OK = CRC3_OK;
    CRC_read = take_from_15th_bit(frame_pos_CRC3, data.data);
    CRC_calc = calc_CRC(CRC3_win_left, data.data);
    ...
    //-------------------------------------------------------
    old_CRC4_OK = CRC4_OK;
    CRC_read = take_from_15th_bit(frame_pos_CRC4, data.data);
    CRC_calc = calc_CRC(CRC4_win_left, data.data);
    ...
    //-------------------------------------------------------
    old_CRC5_OK = CRC5_OK;
    CRC_read = take_from_15th_bit(frame_pos_CRC5, data.data);
    CRC_calc = calc_CRC(CRC5_win_left, data.data);
    ...
    //-------------------------------------------------------
    old_CRC6_OK = CRC6_OK;
    CRC_read = take_from_15th_bit(frame_pos_CRC6, data.data);
    CRC_calc = calc_CRC(CRC6_win_left, data.data);
    ...
    //-------------------------------------------------------
    old_CRC7_OK = CRC7_OK;
    CRC_read = take_from_15th_bit(frame_pos_CRC7, data.data);
    CRC_calc = calc_CRC(CRC7_win_left, data.data);
    ...
    //-------------------------------------------------------
    old_CRC8_OK = CRC8_OK;
    CRC_read = take_from_15th_bit(frame_pos_CRC8, data.data);
    CRC_calc = calc_CRC(CRC8_win_left, data.data);
...
void akp_check_state::set_state(const TDataPocket &data)
{
    ...
    encode_Rx_type             (data);
    encode_freq_value          (data);
    encode_delay_value         (data);
    encode_Ku_value            (data);
    encode_tool_type           (data);
    encode_mode_number         (data);
    encode_calibration_vk      (data);
    encode_tool_no             (data);
    encode_soft_version        (data);
